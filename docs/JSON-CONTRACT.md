...
| 9 × `claude-code`, 2 × `chatgpt-export`, 1 × `gemini-cli` | No assistant turn at all — a session of slash commands, or a `New chat` opened and abandoned. There was nothing for a model to have produced. |
| 7 × `chatgpt-export`, 1 × `claude-code` | Assistant turns that declared no model. The field is optional in both formats and a scattering of conversations simply do not carry it, spread across 2023 and 2026 rather than confined to one old export — which is why this cannot be typed non-optional on the evidence of the other 4,406. |
